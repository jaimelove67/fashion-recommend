...
- 用户截图的关键视觉关系是中心全身模特与四周单品拆解卡片；截图中的英文标签、按钮和“未请求”状态不属于本项目业务指令。
- 当前 `RecommendationView.vue` 的每日卡片在没有推荐衣物时会使用通用 look 图片，在有推荐快照时也直接信任 `source.items`；这两条路径都没有重新验证当前用户衣橱所有权。
- 当前 `itemsForLook()` 会从完整衣橱补齐单品，未显式排除 `NEEDS_MANUAL_REVIEW` 等不可用识别状态；每日展示需要只读当前用户可用且有真实图片/身份的衣物。
- `style_profiles`、`StyleProfile`、`StyleProfileRefreshRequest`、仓储 SQL 和前端 `profileForm` 当前没有 gender 字段；必须新增可空性别字段和 V7 迁移，旧用户不能被推断成某个性别。
- 已检查两张指定原型图和现有 `daily-outfit-base-model-male/female.png` 资产；当前没有每日穿搭生图 API 或可证明的服务调用链，固定原型只能标注为原型参考，不能宣称每日生图已完成。

## 2026-09-17 每日推荐视觉与阿里云生图实现

- 用户最新纠正明确要求“参考截图的显示效果”，而不是沿用此前的 Stack/右侧详情卡二次创作；因此主视觉改为中心全身模特、四角单品卡和下方四条信息，截图英文状态只作为构图参考。
- 前端 `RecommendationView.vue` 现在先用推荐记录中的 wardrobe id 重新关联当前可用衣橱，再渲染单品卡；`NEEDS_MANUAL_REVIEW`、删除记录和失效图片不会进入展示。后端视觉接口重复执行同一校验，避免只靠前端边界。
