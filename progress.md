...
## 2026-09-16 阶段二十七 每日推荐改造启动

- 已读取并遵守当前仓库规划、第一性约束和 `impeccable` 视觉实现规则；新增 `PRODUCT.md` 固化用户、衣橱所有权、四点解释和模特原型边界。
- 已完成每日推荐与档案链路的初步审计：下一步先改性别字段的前后端契约，再改每日推荐的衣橱过滤和参考图式主视觉。
- 当前明确的验证反例：推荐快照包含已从衣橱删除的 id、衣物处于 `NEEDS_MANUAL_REVIEW`、衣橱为空、档案 gender 为空，以及用户 gender 与男女原型选择不一致时，页面都不能展示越界单品或错误性别模特。

## 2026-09-17 阶段二十七 纠偏与运行时生图实现

- 按用户最新反馈移除了“已有卡片再创作”的主视觉路径，`RecommendationView.vue` 改为参考截图的中心模特 + 四角衣物拆解板；四条说明固定为天气适配、场合适配、风格方向、衣橱依据。
- 新增 `BailianImageGenerationClient`、`RecommendationVisualService` 和 `/api/v1/me/recommendations/{id}/visual`：使用指定男女原型资源、当前衣橱校验、Wan 2.6 异步任务创建/轮询，并对未配置/失败状态诚实回退。
- 已完成 `mvn -q -f backend/pom.xml test`、生图客户端专门测试、视觉接口性别/衣橱边界测试和 `frontend npm run build`。Docker 镜像重建完成，backend/frontend/PostgreSQL/MinIO 健康运行。
- 浏览器验收通过：桌面端中央模特 + 四角衣橱单品画板、四点说明均可见；旧 `.ootd-stack-card` 渲染数量为 0；390px 移动端 `scrollWidth` 等于视口宽度，无横向溢出。为避免 QA 触发真实付费生图，本轮容器使用 `BAILIAN_IMAGE_ENABLED=false`，接口返回明确的 `UNAVAILABLE`，默认配置仍支持按需启用阿里云生图。
- 追加私有 `imageObjectKey` 的 Base64 单品参考测试，并重新完成完整 Maven 回归，退出码为 0。
